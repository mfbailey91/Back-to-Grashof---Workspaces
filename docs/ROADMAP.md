...
H13A analytical `c`-domain dispatch, H13B projected-seed discovery, and H13C honest trace termination exist on a separate config; the H12 compact hub and frozen config remain authoritative. H13D–H13F remain unimplemented. L5 stays `parent_incomplete`. Direct-column metrics and natural-leaf parameters are not retuned while source control is blocked. `RETURNED_SET_FOUND` is declared-budget evidence; `COMPONENT_COMPLETE` stays reserved.
