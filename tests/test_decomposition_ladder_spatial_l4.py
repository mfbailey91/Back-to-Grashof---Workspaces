...
def test_l4_exact_u_bundle_promotes_scoped_closed_mechanism() -> None:
    ...
    assert bundle.certificate.closed_mechanism_status is CertificateStatus.EXACT_ON_COMPONENT
    ...
    assert bundle.reconstruction.certificate_status is CertificateStatus.EXACT_ON_COMPONENT
...
def test_l4_catalog_fiber_status_is_scoped_exact_on_component() -> None:
    fiber = next(spec for spec in DEFAULT_FIBER_SPECS if spec.rung is LadderRung.L4)
    assert fiber.certificate_status is CertificateStatus.EXACT_ON_COMPONENT
    ...
    assert "exact_global" in notes  # notes deny multi-component EXACT_GLOBAL
