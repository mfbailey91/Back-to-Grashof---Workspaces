"""Sprint 04 HTML readout assembly."""
...
def assemble_sprint04_payload(results_root: Path) -> dict[str, Any]:
    experiments = []
    commits: list[str] = []
    source_ids: list[str] = []
    ...
    return {
        "title": "Sprint 04 — Pointing manifold",
        "subtitle": "Local fixed-position continuation and discriminating SUUR tests",
        "brand": "aligned terminal-roll",
        "sprint_status": "Implementation complete / Check-in 4 draft",
        "milestone": "M4 — Two-dimensional pointing surface",
        "warning": M4_WARNING,
        "claim": (
            "Predictor-corrector continuation of p(q)=p0 with q6 constant produces a local "
            "2D regular subset with rank-two pointing motion. SUUR φ is defined exactly when "
            "intersecting-axis pairs persist, and the old N_red embedding test is non-discriminating."
        ),
        "pass_count": pass_count,
        "experiment_count": len(experiments),
        "checkin_interpretation": "SUPPORTED",
        "checkin_rationale": (
            "Intersecting-pair distances persist away from home, φ is defined only on that "
            "geometry, and GenericAligned6R remains a negative control where the deprecated "
            "compound-tangent test still passes. Local continuation patches on "
            "IntersectingPairsAligned6R and URLikeAligned6R stay near p0 with rank-two pointing "
            "on the regular subset. This is a local C10 patch, not a fiber or spherical RRRR."
        ),
        ...
        "next_stage": (
            "Pending human Check-in 4. If approved, next stage is an explicit one-dimensional "
            "fiber constraint. Spherical RRRR and exact UR remain blocked."
        ),
        "reproduce": [
...
def write_sprint04_readout(results_root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    (out_dir / "index.html").write_text(render_sprint04_html(payload), encoding="utf-8")
    return payload


def render_sprint04_html(payload: dict[str, Any]) -> str:
    cards = []
    for exp in payload["experiments"]:
        imgs = "".join(
...
<head>
  ...
  <style>
    :root {{
      ...
      background: radial-gradient(ellipse 80% 50% at 10% -10%, #243a28 0%, transparent 55%),
...
<body>
  <div class="shell">
    ...
    <section class="claim"><strong>Check-in 4 interpretation.</strong> {escape(payload["checkin_interpretation"])}.
    ...
    <section class="stats">
      ...
      <div class="stat"><div class="muted">Check-in 4</div><div>{escape(payload["checkin_interpretation"])}</div></div>
