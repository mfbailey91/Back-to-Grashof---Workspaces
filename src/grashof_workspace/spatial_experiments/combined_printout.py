"""Combined printable readout for ATR Sprints 01–04."""
...
CHECKIN_ROWS = (
    ...
    ("4", "M4 Pointing manifold", "SUPPORTED (draft)", "CONTINUE (draft)", "Human gate"),
)


def assemble_combined_payload(results_root: Path) -> dict[str, Any]:
    sprints = []
    for dirname in SPRINT_DIRS:
        payload = json.loads((results_root / dirname / "readout.json").read_text(encoding="utf-8"))
        payload["_dir"] = dirname
        sprints.append(payload)
    total_pass = sum(int(s["pass_count"]) for s in sprints)
    total_exp = sum(int(s["experiment_count"]) for s in sprints)
    return {
        "title": "Aligned terminal-roll — Sprints 01–04",
        ...
        "sprints": sprints,
        "checkins": [
            {"n": n, "milestone": milestone, "interpretation": interp, "decision": decision, "status": status}
            for n, milestone, interp, decision, status in CHECKIN_ROWS
        ],
        "blocked": [
            "fibers / h(q)=c until Check-in 4",
            "spherical RRRR",
        ...
        "reproduce": [
            ...
            "python scripts/generate_atr_sprint01_04_printout.py",
        ],
    }


def write_combined_printout(results_root: Path, out_dir: Path) -> dict[str, Any]:
    payload = assemble_combined_payload(results_root)
    ...
    for sprint in payload["sprints"]:
        ...
        for exp in sprint["experiments"]:
            copied = []
            figure_keys = exp.get("figures") or []
    ...
    (out_dir / "index.html").write_text(render_print_html(payload), encoding="utf-8")
    return payload
...
def render_print_html(payload: dict[str, Any]) -> str:
    ...
    for sprint in payload["sprints"]:
        exp_rows = "".join(
            ...
            f"<td class='pass'>{escape(exp['status'])}</td>"
            f"<td>{escape(exp['expected'])}</td>"
            f"<td>{escape(exp['observed'])}</td>"
            "</tr>"
            for exp in sprint["experiments"]
        )
        figs = []
...
<body>
  <div class="shell">
    ...
    <p class="muted">{escape(payload["subtitle"])}. Twenty experiments across four sprints. Check-in 4 remains a human gate.</p>
    <section class="stats">
      <div class="stat"><span class="muted">Experiments</span><b>{payload["pass_count"]}/{payload["experiment_count"]} PASS</b></div>
      <div class="stat"><span class="muted">Sprints</span><b>01–04</b></div>
      <div class="stat"><span class="muted">Approved gates</span><b>Check-ins 1–3</b></div>
      <div class="stat"><span class="muted">Open gate</span><b>Check-in 4</b></div>
    </section>
